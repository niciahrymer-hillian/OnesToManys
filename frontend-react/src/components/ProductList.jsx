    ...
    <div className="product-grid" role="table" aria-label="Products">
      ...
      {products.map((product) => (
        <div className="product-row" role="row" key={product.product_id}>
          ...
          <span role="cell">{product.units_sold}</span>
          <span role="cell">${Number(product.revenue).toFixed(2)}</span>
          <span role="cell">{product.order_origin}</span>
          <span role="cell">{product.sales_channel}</span>
